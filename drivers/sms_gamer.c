/*
 * sms_gamer.c: driver for SMS Gamer UPSes with real battery voltage reading
 *
 * Based on sms_ser.c by Alex W. Baule <alexwbaule@gmail.com>
 * Modified (C) 2025 - SMS Gamer Driver (Real Battery Voltage + LiFePO4 Support)
 *
...
#define DRIVER_NAME	"SMS Gamer UPS driver"
#define DRIVER_VERSION	"1.06-gamer"
...
/* Tipo de bateria: 0 = Lead-Acid (padrão), 1 = LiFePO4 */
...
void sms_parse_results(uint8_t *rawvalues, SmsData *results) {
    ...
    /* MODIFICAÇÃO: Bytes 11-12 contêm a TENSÃO REAL da bateria, não o percentual */
    memset(buf, 0, BUFFER_SIZE);
    sprintf(buf, "0x%02x%02x", (unsigned char)rawvalues[11], (unsigned char)rawvalues[12]);
    v = strtol(buf, NULL, 16); /* 16 == hex */
    h = v / 10;
    results->batteryVoltage = h;  /* Armazena tensão real */
    
    /* Calcula SOC com base no tipo de bateria configurado */
    if (battery_type == 1) {
        /* LiFePO4 8S (25.6V nominal) */
        /* Curva de descarga LiFePO4: 29.2V (100%), 26.4V (90%), 25.6V (50%), 24.0V (10%) */
        if (h >= 29.2) {
            results->batterylevel = 100.0;
        } else if (h >= 26.4) {
            results->batterylevel = 90.0 + ((h - 26.4) / (29.2 - 26.4)) * 10.0;
        } else if (h >= 25.6) {
            results->batterylevel = 50.0 + ((h - 25.6) / (26.4 - 25.6)) * 40.0;
        } else if (h >= 24.0) {
            results->batterylevel = 10.0 + ((h - 24.0) / (25.6 - 24.0)) * 40.0;
        } else {
            results->batterylevel = 0.0;
        }
        upsdebugx(LOG_DEBUG, "Battery type: LiFePO4, voltage: %.2fV, calculated SOC: %.1f%%", results->batteryVoltage, results->batterylevel);
    } else {
        /* Lead-Acid 2x12V (24V nominal) - Padrão */
        /* Curva de descarga Lead-Acid: 27.6V (100%), 25.2V (75%), 24.0V (50%), 22.8V (25%), 21.0V (0%) */
        if (h >= 27.6) {
            results->batterylevel = 100.0;
        } else if (h >= 25.2) {
            results->batterylevel = 75.0 + ((h - 25.2) / (27.6 - 25.2)) * 25.0;
        } else if (h >= 24.0) {
            results->batterylevel = 50.0 + ((h - 24.0) / (25.2 - 24.0)) * 25.0;
        } else if (h >= 22.8) {
            results->batterylevel = 25.0 + ((h - 22.8) / (24.0 - 22.8)) * 25.0;
        } else if (h >= 21.0) {
            results->batterylevel = 0.0 + ((h - 21.0) / (22.8 - 21.0)) * 25.0;
        } else {
            results->batterylevel = 0.0;
        }
        upsdebugx(LOG_DEBUG, "Battery type: Lead-Acid, voltage: %.2fV, calculated SOC: %.1f%%", results->batteryVoltage, results->batterylevel);
    }
    
    /* Garante que o SOC fique entre 0 e 100 */
    if (results->batterylevel > 100.0) results->batterylevel = 100.0;
    if (results->batterylevel < 0.0) results->batterylevel = 0.0;
    
    /* Validação de tensão da bateria */
    if (results->batteryVoltage < 10.0 || results->batteryVoltage > 60.0) {
        upsdebugx(LOG_WARNING, "Invalid battery voltage: %.2fV, ignoring update", results->batteryVoltage);
        /* Mantém valores anteriores ao invés de zerar */
    }
    ...
    if (results->temperatureC < -20.0 || results->temperatureC > 100.0) {
        upsdebugx(LOG_WARNING, "Invalid temperature: %.2fC, ignoring update", results->temperatureC);
    }
    
    /* Validação de tensões de entrada/saída */
    if (results->inputVac < 0.0 || results->inputVac > 300.0) {
        upsdebugx(LOG_WARNING, "Invalid input voltage: %.2fV", results->inputVac);
    }
    if (results->outputVac < 0.0 || results->outputVac > 300.0) {
        upsdebugx(LOG_WARNING, "Invalid output voltage: %.2fV", results->outputVac);
    }
...
void upsdrv_initinfo(void) {
    ...
    if (get_ups_nominal() == 0) {
        ...
        dstate_setinfo("battery.voltage", "%.2f", DeviceData.batteryVoltage);  /* MODIFICADO: Usa tensão real */
        dstate_setinfo("battery.chemistry", "%s", (battery_type == 1) ? "LiFePO4" : "PbAc");
        dstate_setinfo("battery.type", "%s", (battery_type == 1) ? "LiFePO4" : "Lead-Acid");
...
void upsdrv_help(void) {
    printf("SMS Gamer UPS driver for Network UPS Tools\n\n");
    printf("This driver supports SMS Gamer UPS models that send real battery voltage.\n\n");
    printf("Configuration options (ups.conf):\n");
    printf("  batterytype=<type>  Battery type: lifepo4 or leadacid (default: leadacid)\n");
    printf("  rebootdelay=<sec>   Reboot delay in seconds (default: 64)\n\n");
    printf("Supported models:\n");
    printf("  - SMS Gamer 2000VA (tested with LiFePO4 8S battery)\n");
    printf("  - Other SMS Gamer models (potentially compatible)\n\n");
    printf("For more information, see:\n");
    printf("  - README_PT.md (Portuguese documentation)\n");
    printf("  - README_EN.md (English documentation)\n");
}
...
void upsdrv_makevartable(void) {
    ...
    snprintf(msg, sizeof msg, "Battery type: leadacid or lifepo4 (default=leadacid).");
    addvar(VAR_VALUE, "batterytype", msg);
}
...
void upsdrv_initups(void) {
    ...
    /* Configura o tipo de bateria */
    if ((val = getval("batterytype"))) {
        if (!strcasecmp(val, "lifepo4")) {
            battery_type = 1;
            upsdebugx(LOG_INFO, "Battery type set to: LiFePO4");
        } else if (!strcasecmp(val, "leadacid")) {
            battery_type = 0;
            upsdebugx(LOG_INFO, "Battery type set to: Lead-Acid");
        } else {
            upslogx(LOG_WARNING, "Invalid battery type '%s', using default (Lead-Acid)", val);
            battery_type = 0;
        }
    } else {
        upsdebugx(LOG_INFO, "Battery type not specified, using default: Lead-Acid");
        battery_type = 0;
    }
}
